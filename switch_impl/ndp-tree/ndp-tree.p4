...
@name(".buffersense") register<bit<16>, bit<2>>(32w4) buffersense;

control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @name(".decreasereg") action decreasereg() {
        buffersense.read(meta.meta.register_tmp, (bit<2>)(bit<2>)standard_metadata.egress_port);
	buffersense.write((bit<2>)standard_metadata.egress_port, (meta.meta.register_tmp-1 + (bit<16>)standard_metadata.priority-1));
    }
    ...
    @name(".dec_counter") table dec_counter {
        ...
        key = {
            ...
            standard_metadata.priority: range;
	}
        size = 2;
    }
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    @name(".set_dmac") action set_dmac(bit<48> dmac, bit<48>smac) {
    ...
    @name(".setpriolow") action setpriolow() {
        buffersense.read(meta.meta.register_tmp, (bit<2>)(bit<2>)standard_metadata.egress_port);
        buffersense.write((bit<2>)(bit<2>)standard_metadata.egress_port, (bit<16>)(meta.meta.register_tmp + 16w1));
	...
	//if (true) {
	    ...
	    //log_msg("ndp flag: {}", {hdr.ndp.flags});
	    if (hdr.ndp.flags == 256) {
